...
3. **综合判断**：faithfulness +0.179 是实质性改善（新数据源解决了幻觉问题），precision/recall 的下降主要是**评估方法与新数据类型不完全匹配**导致，而非检索质量真的变差。

**v14 验证结果（已实施上述改进）**

| 指标 | v13 | v14 | Δ | 结论 |
|------|-----|-----|---|------|
| faithfulness | 0.713 | 0.710 | -0.003 | 持平（compute tool import 修复带来稳定性）|
| answer_relevancy | 0.930 | 0.952 | **+0.022** ✅ | compute 错误减少，答案更完整 |
| context_precision | 0.571 | 0.622 | **+0.051** ✅ | 月度聚合有效，方向正确 |
| context_recall | 0.590 | 0.490 | -0.065 ⚠️ | 见下方分析 |
| ragas_score | 0.707 | 0.694 | -0.013 | 综合略降 |

**v14 recall 仍未恢复的原因**：v14 的 recall 数值（0.490）低于 v13（0.590），原因是 v14 eval 使用了更新后的 Set D ground_truth（更具体的数值），导致 Judge 原子分解出更多需要核对的 claim，而部分 claim（如"FEDFUNDS 从 0.08% 升至 4.33%"）需要精确的宏观数据在 context 中出现，若 Planner 未检索到对应的 FRED 数据行，recall 就会被扣分。这是**ground_truth 数值化使评估更严格**的副作用，而非检索能力退步。

**后续方向**：进一步优化 Planner 对 D03 类问题的路由（同时检索 price_history + macro_indicators），确保 FEDFUNDS 数据出现在 context 中。

**教训**：引入结构化时序数据（日线价格）时，评估指标的适配需要同步考虑。Precision@K 假设每个 context 条目是一个独立的文本 chunk，对"一行=一个时间点"的时序数据不适用。应在引入新数据源时同步审查 eval 的 format_context 函数，考虑聚合后再送 Judge。同时，ground_truth 数值化会使评估更严格（好事），但需要同步确保检索覆盖所有涉及的数据源。

---

## 观察 #20：yfinance 1.3.0 API 变更导致 earnings_history 全为空、pe_ratio 全为 NULL

**版本**：v13–v14（数据入库阶段）

**现象**

- `earnings_history` 表仅有 6 行（2024–2026），`eps_actual` / `eps_estimate` 全部为 NULL
- `price_history` 2854 行，`pe_ratio` 全部为 NULL（0/2854 非空）
- D01（P/E 历史估值）和 D02（Q3 2023 EPS 超预期）在 eval 中 context_items=0 或 0 recall

**根因**

`ingest_prices.py::fetch_earnings_history` 使用 `tk.quarterly_earnings`，而该属性在 yfinance 1.3.0 中已弃用并返回 `None`。代码中的列名检查（`"Reported EPS"`、`"EPS Estimate"`）永远不执行，导致所有行的 EPS 字段为 NULL。

`compute_pe_ps_ratios` 依赖 `eps_actual` 的 4 季度滚动 TTM，全 NULL 时永远无法计算 P/E。

**修复**（`ingestion/ingest_prices.py`）

- 将 `tk.quarterly_earnings` 替换为 `tk.get_earnings_dates(limit=40)`
- 新 API 返回列：`['EPS Estimate', 'Reported EPS', 'Surprise(%)']`，index 为财报公告日期
- 新增 `_ann_date_to_quarter_end()` 将公告日期映射到财季末（Jan-Mar→上年Q4，Apr-Jun→Q1，Jul-Sep→Q2，Oct-Dec→Q3）
- 修复后覆盖 50 行（2014–2026），pe_ratio 全部有值（区间 16.13–53.98）
- Q3 2023 数据：eps_actual=1.55，eps_estimate=1.45，surprise=7.05%（与 D02 ground_truth 完全吻合）

**教训**：yfinance 属性在大版本升级时经常静默返回 `None` 或改变列名，不抛异常。入库脚本需在每次 yfinance 升级后验证关键字段的非空率，而非仅检查行数。

---

## 观察 #21：Synthesizer 在大 context 下将相关性推断为因果关系（B01 类幻觉）

**版本**：v14 → v15

**现象**

B01 题目"Fed 加息如何影响 Google 广告收入增长"，context 包含 49 个条目（FEDFUNDS 月度数据 + 多个 SEC 广告收入 chunk），faithfulness=0.0。

Judge 判断：答案的核心论点（利率上升 → 广告主削减预算 → Google 广告收入下降）引用了 context 中存在的 [39][40]，但这两个 chunk 的内容根本没有陈述这个因果机制。Synthesizer 把数据的时间相关性（2022 年利率上升 + 2022 年广告收入承压）识别为背景知识中的因果关系，并伪装成 context 引用。

**根因**

原 Rule 1 要求"causal claim 要引用"，但没有区分"cited source 明确陈述了因果机制"和"cited source 只是时间上相关的数据"。LLM 用训练知识填补了因果链，同时找了一个真实存在的 context 编号来引用。

**修复**（`agent/synthesizer.py`）

将 Rule 1 拆分为两条独立规则：

- **NUMBERS AND DATES**：数值必须在 cited source 中原文存在，不得召回背景知识
- **CAUSAL CLAIMS**（新规则）：因果声明需 context 明确陈述机制；相关性数据≠因果性；若无明确声明则必须写"The provided context does not establish a direct causal link between X and Y."

修复后 B01 答案改为正确表述："context 不建立 Fed 加息与广告收入之间的直接因果联系"，faithfulness 从 0.0 升至 1.0。

**教训**：RAG 系统的 faithfulness 最难防的幻觉不是凭空捏造，而是用训练知识做因果推断并配上真实存在的引用编号。需要明确区分"数值引用"和"因果机制引用"两类规则。

---

## 观察 #22：Critic missing_hint 未传递给 Synthesizer，导致已知缺口仍被幻觉填补

**版本**：v14 → v15

**现象**

A04 题目"Google Cloud FY2023 年收入"，Critic 在每轮迭代后均明确报告"context 不含 Google Cloud 全年收入总额"，但 Synthesizer 仍然输出"$33,088 million [1]"并配上错误引用（faithfulness=0.0）。

**根因**

`per_loop.py` 的 PER Loop 在 `synthesize()` 调用时未传入 `missing_hint`：
...
Synthesizer 看不到 Critic 的诊断结果，只能根据 context 和自身训练知识生成答案。`$33,088M` 是 LLM 高置信的训练先验，在没有明确约束的情况下会直接输出。

即使在 Synthesizer system prompt 中加了 Rule 1（数值必须在 context 中原文存在），因为 LLM "认为" chunk [1] 里有这个数字，规则形同虚设。

**修复**（两处）

1. `synthesizer.py`：`synthesize()` 增加 `missing_hint` 参数，将其格式化为 `RETRIEVAL GAP` 块，**置于 user message 最前**（在 context 之前），内容为：
   ```
...
2. `per_loop.py`：调用 `synthesize()` 时传入最终 `missing_hint`：
   ```python
   answer = synthesize(question, all_context, llm, max_tokens=cfg.llm.max_tokens, missing_hint=missing_hint)
   ```

**关键设计决策**：将 `RETRIEVAL GAP` 放在 context 之前而非之后。测试发现放在末尾时 LLM 忽略约束（已知训练先验覆盖了末尾指令），放在最前时约束有效。

修复后 A04（单轮 context 不含完整数字时）输出："The provided context does not contain the total revenue figure for Google Cloud for the full fiscal year 2023."，faithfulness=1.0。

**教训**：Critic 是 PER Loop 中最了解 context 缺口的组件，但如果这个诊断结果不传给下游的 Synthesizer，整个检测就是无效的。架构上 Critic → Synthesizer 的信息流应该是显式的，不能只靠 LLM 自行判断 context 的完整性。
