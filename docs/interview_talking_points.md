...
| 指标 | v1 | v11（eval 方法论） | v12（section 修复） | v1→v12 Δ |
|------|----|--------------------|---------------------|----------|
| context_precision | 0.174 | 0.603 | **0.688** | **+0.514** |
| context_recall | 0.471 | 0.587 | **0.651** | **+0.180** |
| faithfulness | 0.618 | 0.544 | **0.667** | +0.049 |
| answer_relevancy | 1.000 | 0.944 | **0.972** | -0.028 |
| ragas_score | 0.566 | 0.670 | **0.741** | **+0.175** |

v11→v12 最大提升是 `faithfulness` +0.123：section 修复后 MD&A（2→30 chunk）、Risk Factors（0→34 chunk）的实质内容进入检索池，Synthesizer 的答案声明可以得到真实 context 支撑，而不是目录片段。
